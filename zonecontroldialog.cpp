...
//    ui->Z4InputComboBox->addItem("MHL", 48);
    ...
    connect(&m_Comm,SIGNAL(ZonePower(int, bool)),this,SLOT(ZonePower(int, bool)));
    connect(&m_Comm,SIGNAL(ZoneInput(int, int)),this,SLOT(ZoneInput(int, int)));
    connect(&m_Comm,SIGNAL(ZoneVolume(int, int, QString)),this,SLOT(ZoneVolume(int, int, QString)));
    connect(&m_Comm,SIGNAL(ZoneMute(int, bool)),this,SLOT(ZoneMute(int, bool)));
...
void ZoneControlDialog::ZonePower (int zone, bool on)
{
    if (zone == 2)
    {
...
void ZoneControlDialog::ZoneVolume (int zone, int/* volume*/, QString asString)
{
    if (zone == 2)
    {
        ui->Z2VolumeLineEdit->setText(asString);
    }
    else if (zone == 3)
    {
        ui->Z3VolumeLineEdit->setText(asString);
    }
}

void ZoneControlDialog::ZoneMute (int zone, bool muted)
{
    if (zone == 2)
    {
        ui->Z2MuteButton->setChecked(muted);
    }
    else if (zone == 3)
    {
        ui->Z3MuteButton->setChecked(muted);
    }
}

void ZoneControlDialog::DisplayData(int DispNo, QString data)
{
    if (DispNo == 2)
    {
